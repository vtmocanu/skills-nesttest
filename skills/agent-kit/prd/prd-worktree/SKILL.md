...
> Vendored from [vfarcic/dot-ai](https://github.com/vfarcic/dot-ai) `.claude/skills/dot-ai-worktree-prd/` (MIT, Copyright (c) 2025 Viktor Farcic). Original author: Viktor Farcic. Renamed from `worktree-prd`.
...
```bash
bash "<this skill's directory>/create.sh" [number] "[title]"
```

Otherwise let the script look it up from `prds/`:
```bash
bash "<this skill's directory>/create.sh" [number]
```
